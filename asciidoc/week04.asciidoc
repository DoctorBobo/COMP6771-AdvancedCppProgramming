...
template <typename T>
smart_ptr<T>& smart_ptr<T>::operator=(const smart_ptr<T>& o) // d is passed by value, which invokes the copy constructor
{
   smart_ptr<T>{o}.swap(*this);  // resources of our old smart_ptr go into the temporary,
                                 // which is cleaned up at the semicolon by default
   return *this;
}

template <typename T>
void smart_ptr<T>::swap(smart_ptr<T>& o) noexcept
{
   using std::swap; // very important!
   swap(data, o.data_);
   swap(size_, o.size_);
}
------------
...
* This is the preferred copy assingment operator.
   ...
   - That's when you go back to the original check-for-self copy assignment operator, with a caveat:

[source,cpp]
------------
// special optimisation assignment for dumb arrays
template <typename T>
smart_ptr<T>& smart_ptr<T>::operator=(const smart_ptr<T>& o)
{
   if (this != &d) {
      if (o.size_ == size_) // saves an expensive heap allocation if it isn't necessary
         std::copy(o.begin(), o.end(), begin());
      else
         smart_ptr<T>{}.swap(*this);
   }
...
* This last one is an optimisation: **do not do optimise without performance testing**.
   - If you do, you might pessimise your code.

=====

* [big]#**The rule of three states that if you have a need to implement one of these, you must implement
  all of them.**#
[WARNING]
=========
...
* link:http://stackoverflow.com/a/9322542/4097528[StackOverflow: Howard Hinnant's answer to
...
[source,cpp]
------------
int&& rvalue_reference = 0; // okay, deliberately _not_ `auto&&`
const int&& rvalue_reference_to_const = 0; // legal, but the rvalue reference is now useless.
------------
...
------------
template <typename T>
smart_ptr<T>::smart_ptr(smart_ptr<T>&& o) noexcept // move constructor must always be noexcept
   : size_{o.size_},
     data_{o.data_}
{
...
------------
template <typename T>
magic<T>::magic(magic<T>&& o) noexcept
...
------------
template <typename T>
magic<T>& magic<T>::operator=(magic<T>&& m) noexcept
{
   if (this != &r)
   {
      delete[] resource_;
...
* Howard Hinnant (the man that single handedly wrote clang's libc++ from scratch) argues that you
   ...
   ** If you haven't received a reference to a temporary, then the client has a bug. It's _their_
      fault, and _their_ problem to fix.
* Patrice Roy (another member of WG21) taught me during my adventures at CppCon the same thing.
...
------------
template <typename T>
magic<T>& magic<T>::operator=(magic<T>&& m) noexcept // move assignment operator must always be noexcept
{
   assert(this != &r);
   clear();
   swap(m);
...
template <typename T>
magic<T>& magic<T>::clear() noexcept
{
   delete[] resource_;
   data_.clear();
}
...
template <typename T>
void magic<T>::swap(magic<T>& o) noexcept // swap must always be noexcept!!
{
   using std::swap;
   swap(resource_, o.resource_);
   swap(data_, o.data_);
}
------------

* This way, the client is forced to fix their problem, rather than dart around it.
* If you read _The {cpp} Programming Language (4th Edition)_, you'll also find the above function.
=========
...
* [big]#**If your type doesn't personally manage a resource, don't bother with _any_ of the five.**#
