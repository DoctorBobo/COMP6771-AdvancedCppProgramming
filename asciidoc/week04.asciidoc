...
* Compilers are already aware that returning a local variable can be 'moved', _without_ the need
  for `std::move`.
* This is known as _copy elision_.
   ...
   - When these conditions aren't met, the compiler will automatically perform a move anyway.
