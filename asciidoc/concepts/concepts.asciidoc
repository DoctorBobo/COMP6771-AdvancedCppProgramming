...
== Concepts **now**!
While concpets are only just beginning to enter the world of {cpp}, their reasoning has been in
circulation since at least 2014. Bjarne Stroustrup has been recommending for years that programmers
include these requires clauses at the top of their concepts, so that when a championing compiler
arrives, they can use concepts immediately without program redesign and reevaluation.
...
[TIP]
=====
* link:https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Rt-concept-def[CppCoreGuidelines
