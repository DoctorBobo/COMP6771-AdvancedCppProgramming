...
* The `priority_queue` adaptor adapts `vector` by default.
...
* link:http://en.cppreference.com/w/cpp/container/priority_queue[cppreference --
  `std::priority_queue`]
...
* This is opposed to Java, which works with _references_ by default.
...
* Usually not what you want for composite types.
   - If you do want to pass a composite type by value, document it, explaining why a _copy_ is
     necessary.
   - Passing a composite type by value is usually done when you want to make local changes to an
     object that should not be handed back to the caller.
   - Putting `const` in front of a composite type that has been passed by value is probably a
...
* Usually what you want for composite types.
   - If you don't know how you want to pass a composite type, pass by reference-to-`const` until
     you work it out.
...
* As with all the other containers, you need to have a valid reason to use `deque` over `vector`.
   - Wanting a container of `bool` is a valid reason to use a `deque` over a `vector`.
* Understand that both the `stack` and `queue` adaptors (not in these notes) wraps around a `deque`
  _by default_.
...
* link:http://en.cppreference.com/w/cpp/container/stack[cppreference -- `std::stack`]
...
[TIP]
=====
* Stroustrup, B. _The {cpp} programming language_. Fourth edition. p.913 -- 920. Pearson Education,
...
[TIP]
=====
* Stroustrup, B. _The {cpp} programming language_. Fourth edition. p.909 -- 913. Pearson Education,
  Inc. Upper Saddle River, NJ.
...
* Stroustrup, B. _Programming: principles and practice using {cpp}_. Second edition. p.757 -- 801.
  Pearson Education, Inc. Upper Saddle River, NJ.
* Sutter, H. _More exceptional {cpp}: 40 new engineering puzzles, programming problems, and
  solutions_. p.53 -- 59. 2002. Pearson Education, Inc. Boston, MA.
...
* Scott Meyers gives three conditions for using emplacement without benchmarking:
...
* If you cannot satisfy all three of his conditions, you will need to benchmark by writing code
  with emplacement, and then changing it to insertion (`push_back` and `push_front` are both
  insertion).
...
* Another type of `OutputIterator` that you should become familiar with is the `back_insert_iterator`.
* We can simplify our `make_crowd` function above to the following:
...
std::vector<int> make_crowd(const int children, const int adults, const int seniors)
{
   ...
   distribute = std::uniform_int_distribution<>{18, 64};
   std::generate_n(std::back_insert_iterator<decltype(crowd)>{crowd}, adults,
                   [&]{ return distribute(generator); });

   distribute = std::uniform_int_distribution<>{65, 128};
   std::generate_n(std::back_insert_iterator<decltype(crowd)>{crowd}, seniors,
                   [&]{ return distribute(generator); });
...
* Like with everything else, unless you _know that you intend_ to change the pointee's value, stick
...
* All standard library containers that support `BidirectionalIterator` also have a
