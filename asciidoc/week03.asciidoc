...
std::vector<int> make_crowd(const int children, const int adults, const int seniors)
{
   ...
   std::generate_n(std::back_insert_iterator<decltype(crowd)>{v}, children,
                   [&]{ return distribute(generator); });

   distribute = std::uniform_int_distribution<>{18, 64};
   std::generate_n(std::back_insert_iterator<decltype(crowd)>{v}, adults,
                   [&]{ return distribute(generator); });

   distribute = std::uniform_int_distribution<>{65, 128};
   std::generate_n(std::back_insert_iterator<decltype(crowd)>{v}, seniors,
                   [&]{ return distribute(generator); });
